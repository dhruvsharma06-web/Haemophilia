...
def calculate_arm_trajectory(landmarks):
    ...
    return (
        float((le.x - ls.x) / torso_length),
        float((re.x - rs.x) / torso_length),
    )
...
def get_rule_error(
    ...
    relative_tilt = abs(
        torso_tilt - baseline_torso_tilt
    )
...
def save_error_frame(
    ...
    """Write the same red-skeleton error artifact as the demo."""
...
class AssistedShoulderFlexionAssessment:
    """Per-video/per-session state machine."""
    ...
    def process_frame(
        ...
        Returns a completed-rep dictionary only when a rep completes.
        """
    ...
    def get_live_state(
        ...
        if self.first_error is not None:
            error_type = get_advanced_error_type(
                self.first_error
            )
    ...
    def _complete_rep(
        ...
        if weaker_arm_max < MIN_ARM_TOP_ANGLE:
            ...
            final_error = (
                self._incomplete_arm_error(
                    ...
                    right_max_angle,
                    end_frame,
                    "stopped short. Both arms must reach "
                    "the full overhead position.",
                )
            )
            ...
            final_error = {
                ...
                "feedback": (
                    ...
                    "the repetition. Keep your torso "
            ...
            final_error = {
                ...
                "frame": end_frame,
                "feedback": (
                    "Incorrect: one arm did not stay level "
                    "with the other. Raise both arms together "
        ...
        if final_error is not None:
            ...
            feedback = (
                "Excellent movement. Both arms reached "
                "the range with good control."
            )

        elif score >= 80.0:
            feedback = (
                "Good movement. Keep both arms even "
                "and your torso upright."
            )

        elif score >= 70.0:
            feedback = (
                "Good repetition. Try to make the movement "
                "smoother and more controlled."
            )

        else:
            feedback = (
                "Movement completed. Improve your range "
